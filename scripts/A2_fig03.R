...
f_customplot <- function(dat, selectedParam, outcomeVar) {
  ...
  pplot <- ggplot(data = tempdat) +
   geom_ribbon(aes(
      x = PostInterventionDays, ymin = lower.ci.val / 10000, ymax = upper.ci.val / 10000, fill =  as.factor(MainParam)  ), alpha = 0.5) +
    geom_line(aes(x = PostInterventionDays, y = mean.val / 10000, col = as.factor(MainParam)), size = 1) +
...
for (selected_EIR in c(3,10, 90)){
  ...
  p1 <- f_customplot(plotdat,"developmentSurvival", "Nv0") + 
    labs( title = "A) Development survival")
  p2 <- f_customplot(plotdat,"femaleEggsLaidByOviposit", "Nv0") + 
    labs( y = "", title = "B) Eggs laid per feeding-ovipositing-cycle")
  p3 <- f_customplot(plotdat,"developmentDuration", "Nv0") + 
  ...
  p1 <- f_customplot(plotdat,"developmentSurvival", "Nv") + 
    labs(title = "D) Development survival"  )
  p2 <- f_customplot(plotdat,"femaleEggsLaidByOviposit", "Nv") + 
    labs( y = "", title = "E) Eggs laid per feeding-ovipositing-cycle"  )
  p3 <- f_customplot(plotdat,"developmentDuration", "Nv") +
  ...
  ggsave(paste0("A2_Fig3_EIR-",selected_EIR, ".png"), plot = p123, path = file.path("additional_figures"),width = 18, height = 12, device = "png")
  #ggsave(paste0("A2_Fig3_EIR-",selected_EIR, ".pdf"), plot = p123, path = file.path("additional_figures"),width = 18, height = 12, device = "pdf")
